...
{ 
	...
	FileViewer(){
		...
		frame.setLocationRelativeTo(null); //frame을 가운데 정렬
...
//		con.setLayout(new BorderLayout());
		...
		com.add(ComB,BorderLayout.EAST);//panel에 combobox를 더함
	...
	{
		StringBuffer path=new StringBuffer();//append,stringbuffer 메모리 절약위함
		TreePath temp=e.getPath(); 
		Object list[]=temp.getPath();
		for(int i=0;i<list.length;++i)//list배열 크기만큼 for문을 돌린다
		{
			if(i>0)
			{
	...
	public void treeWillExpand(TreeExpansionEvent e)
	{
		...
		else
		{
			try{
				...
				DefaultMutableTreeNode tempChild;//TreeNode에서 지원
				for(File temp:list)
				{
